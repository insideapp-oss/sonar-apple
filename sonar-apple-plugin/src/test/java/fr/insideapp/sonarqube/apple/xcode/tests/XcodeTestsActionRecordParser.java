...
public class XcodeTestsActionRecordParser {
    ...
    private XcodeActionRecordParser parser;

    @Before
    public void prepare() {
        parser = new XcodeActionRecordParser();
    }

    @Test
    public void parse_oneTestRef() throws IOException {
        // prepare
        File recordFile = new File(baseDir, "record_oneTestRef.json");
        String recordJSON = FileUtils.readFileToString(recordFile, Charset.defaultCharset());
        // test
        final List<ActionRecord> actionsRecord = parser.parse(recordJSON);
        // assert
        assertThat(actionsRecord).hasSize(2);
    }
